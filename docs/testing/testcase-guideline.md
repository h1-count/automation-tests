...
reviewer 收敛后、发起用例确认前，可由宿主表格运行时从标准化评审模型生成 `cases-review.xlsx`（确认前的可选评审辅助；确认 accepted 后的用户侧交付 Excel 见 §7.1）：
...
- 与 §4.3 的关系：§4.3 是确认前的可选评审辅助（`cases-review.xlsx`，宿主表格运行时 + 回执管道）；本节是确认后的强制用户交付（交付版三表投影）。两者互不替代；
