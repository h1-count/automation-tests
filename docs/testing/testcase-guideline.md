...
reviewer 收敛后、发起用例确认前，**必须**由宿主表格运行时从标准化评审模型生成 `cases-review.xlsx`，作为每轮用例确认的用户审核界面（确认后 `accepted` 的用户侧交付 Excel 见 §7.1）：
...
- 与 §4.3 的关系：§4.3 是每轮确认前的强制用户审核界面（`cases-review.xlsx`，宿主表格运行时 + 回执管道，随 cases.md 演进逐轮再生成）；本节是确认后的强制用户交付（交付版三表投影）。两者互不替代；
