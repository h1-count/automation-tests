...
- 内容必须由确认时的套件 `cases.md` 确定性解析生成（`<details>` 块、步骤表逐行投影），不得手改或混入未确认用例；生成后与 `cases.md` 抽样核对行数（用例数、执行行数）；
- Excel 首行或说明区标注「以套件 cases.md 为准」；人工修改后的 Excel 禁止导回正式用例（与 §4.3 只读原则一致）；
- 与 §4.3 的关系：§4.3 是确认前的可选评审辅助（`cases-review.xlsx`，宿主表格运行时 + 回执管道）；本节是确认后的强制用户交付（双表投影，无回执要求）。两者互不替代；
