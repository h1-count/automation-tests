...
- 判断：该文档的平行段落口径冲突位置是项目已知事实；已裁决口径可跨请求复用，避免重复触发裁决链；通用交叉对照方法由 SKILL.md 规则承载，不在此复制。
- 当前优先策略：open-platform 后续用例涉及「测试未通过」状态操作列文案时，直接按已裁决口径【继续开发】（2026-08-19 用户决定，以「产品开发首页P0」段为准）；需求文档版本更新时重新执行平行段落交叉对照（通用方法见 SKILL.md「v7 生成阶段质量检查」）并核对裁决是否仍适用；不复制需求原文。
- 证据引用：testcases/web/open-platform/suites/delete-product/design.md（需求歧义表 F-01 行、评审与正式决定）
- 验证条件：已通过当前证据验证；后续发现同范围冲突时以最新可审查记录更新
- 最近更新：2026-08-20T01:07:58.430Z
