...
- 证据状态：受控探索已验证
- 观察：产品接入系统功能需求说明 v1.2 中，测试未通过状态的操作列文案两处口径冲突：「开发状态统一说明」段写【重新开发】，「产品开发首页P0」段写【继续开发】；按章节提取正文时未自动暴露跨段矛盾，由首轮隔离评审发现（F-01），导致裁决→定稿→复审链（r3 轮，约 10 分钟）。
- 判断：按章节提取正文不会自动暴露平行段落的口径矛盾；交叉对照必须作为生成阶段显式检查项，矛盾在 plan 阶段登记并随用例确认一次裁决，可避免「评审发现→裁决→定稿复审」链式波动。
- 当前优先策略：生成 REQ 时对描述同一对象/同一行为的平行段落逐条对照（统一说明段 ↔ P0 主段落 ↔ 附录）；发现矛盾登记「需求歧义与未定义预期」并中性化断言，不单边采信；已裁决案例：测试未通过操作列按「产品开发首页P0」段【继续开发】定稿（2026-08-19 用户决定）。
- 证据引用：testcases/web/open-platform/suites/delete-product/design.md（歧义表 F-01 行）、skills/iot-automation-testing/SKILL.md（v7 生成阶段质量检查项 1）
- 验证条件：已通过当前证据验证；后续发现同范围冲突时以最新可审查记录更新
- 最近更新：2026-08-20T01:01:04.702Z
