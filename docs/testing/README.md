...
| 主题 | 责任文件 | 负责内容 | 不负责内容 |
| --- | --- | --- | --- |
| 工作流基线 | [workflow-baseline.md](./workflow-baseline.md) | 当前唯一可写 v1、冻结请求策略与调试期硬切换边界。 | 用例字段、运行门禁与环境规则。 |
| 任务生命周期与执行门禁 | [automation-guideline.md](./automation-guideline.md) | v1 复用分支与生命周期（含交付目标首轮确认）、Activity、callback、恢复、Gate、执行清单、定位修复回退与结果沿用、运行和封印门禁。 | 用例字段与质量、定位候选条件、环境并发、报告字段。 |
...
| 测试设计与用例 | [testcase-guideline.md](./testcase-guideline.md) | v3 精简设计索引、`testcase-v1-layered` 单一 `cases.md`、只读 Excel 评审视图、请求内来源、`REQ ↔ RULE ↔ caseId`、候选门禁、语义 reviewer、一次用例确认和确认后交付 Excel。 | 生命周期、reviewer 运行事实、环境/数据生命周期、工程运行和报告字段。 |
...
| 专有契约与版本 | [contract-registry.md](./contract-registry.md) | 当前可执行的专有契约标识与语义责任索引。 | 各契约的业务字段、状态机或执行算法。 |
