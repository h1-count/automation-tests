...
| 主题 | 责任文件 | 负责内容 | 不负责内容 |
| --- | --- | --- | --- |
| 任务生命周期与执行门禁 | [automation-guideline.md](./automation-guideline.md) | 上下文加载、事件状态机、Activity、callback、评审编排、恢复、Gate v2、宿主生命周期适配、统一执行清单、Oracle 运行求值、终态 unknown 与封印门禁。 | selector 风险判定、执行并发、用例业务映射、报告证据字段。 |
...
| 计划与测试用例 | [testcase-guideline.md](./testcase-guideline.md) | 计划内容与正式决定、用例业务状态、用例包完整度、原子用例、覆盖基准、`REQ ↔ RULE ↔ caseId ↔ oracleId`、Oracle 权威来源、reviewer 角色适用性、评审结论与变更影响。 | 任务状态机、Oracle 运行求值、reviewer 运行事实、会话实现、测试数据生命周期、报告证据字段。 |
| UI 定位 | [selector-guideline.md](./selector-guideline.md) | 源码优先证据、无头 selector 验证、可见 Inspector fallback、ARIA 探索、Web/App/WebView 定位优先级与审核。 | 探索生命周期、业务步骤、环境选择。 |
