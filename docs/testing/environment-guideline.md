...
本文规定自动化测试的环境命名、地址管理、选择审批、生产保护、健康检查和测试数据隔离方式，适用于开放平台 Web、管理后台、App、API、MQTT 和 IoT 链路测试。
...
测试计划必须分别记录“本地配置状态”“预检状态”“用户确认状态”和“未闭合项”：

- **已配置**：变量存在或地址可解析；不表示账号可登录、环境可访问或允许执行。
- **已预检**：本地配置结构与会话文件已完成检查；不表示业务服务可用。
- **已确认**：用户明确选择目标环境；这是进入后续执行门禁的必要条件，不能由 `.env` 自动推导。
