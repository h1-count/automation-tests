...
- 运行标识、开始/结束时间和脱敏触发来源。
- 关联的测试范围、`caseId` 汇总与用例/脚本版本摘要；完整清单只保留在证据索引。
- 脚本版本或提交标识；本机仓库路径、分支名、Graphify 路径及其未使用原因不写入报告。
- `TEST_ENV`、环境名称和非敏感目标系统标识。
- Runner 与浏览器/设备版本；完整执行命令仅保留在受控执行日志，不进入报告正文。
- 通过、失败、跳过、阻塞和未知的数量。
- 每个失败/阻塞用例的失败步骤、错误摘要、证据路径和分类。
- `CaseEvidenceBundle` 总索引、证据完整性和脱敏状态；通过用例不在报告正文逐条展开。延期用例单列 readiness blocker 和解除条件。
- 存在定位修复时，记录 incident 摘要、原/新定位、受影响与重跑 case、沿用 case、原授权摘要和新执行清单摘要；不得展示定位之外的页面正文。
- 多阶段用例仅汇总当前等待转换、恢复次数及最终自动验证结果；明细保留在证据索引。待审核、审核通过后晋升、驳回后重新发起和受控残留分别统计。
