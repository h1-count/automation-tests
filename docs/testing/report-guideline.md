...
- 是否完成复盘：用户偏好仅在存在新偏好时更新 `.local/testing-memory.md`；项目经验按唯一责任规范处理，并逐项报告“待验证 / 已验证 / 已覆盖 / 无需更新”及原因。
...
`.local/testing-memory.md` 是 Git 忽略的当前用户持续偏好文件，不共享、不用于 CI。项目经验的完整规则见[项目测试经验规范](./knowledge/README.md)；本节只定义报告如何呈现偏好和经验复盘结果。
