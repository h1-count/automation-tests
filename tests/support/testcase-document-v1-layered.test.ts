...
test("only testcase-v1-layered is parseable while archived formats are unsupported", () => {
  assert.equal(CURRENT_TESTCASE_DOCUMENT_VERSION, "testcase-v1-layered");
  assert.equal(isCurrentTestcaseDocumentVersion("testcase-v1-layered"), true);
  assert.equal(parseTestcaseDocument("> 结构版本：testcase-v1。").version, "unsupported");
  assert.equal(parseTestcaseDocument("> 结构版本：testcase-v1-flat。").version, "unsupported");
  assert.equal(isStructuredTestcaseDocumentVersion("unsupported"), false);
});

test("testcase-v1-layered parses index, details, parameter data, and effective governance", () => {
  const document = parseTestcaseDocument(layeredCases());
  assert.equal(document.version, "testcase-v1-layered");
...
test("testcase-v1-layered rejects inconsistent views and invalid execution contracts", () => {
