...
const currentPlan = `> 结构版本：test-design-index-v3 / rule-design-ledger-v3 / case-relation-projection-v3。
...
test("非当前规则台账和拼错 marker 均硬失败", () => {
  for (const marker of ["rule-design-ledger-v2", "rule-design-ledger-v33", "rule-design-ledger-v3-beta"]) {
    const issues = validateRuleCoverage(currentPlan.replace("rule-design-ledger-v3", marker), []);
    assert.equal(issues[0]?.name, "规则台账契约");
    assert.match(issues[0]?.detail ?? "", new RegExp(marker, "u"));
  }
});
...
test("缺少当前规则台账 marker 时硬失败", () => {
  ...
  assert.equal(issues[0]?.name, "规则台账契约");
  assert.match(issues[0]?.detail ?? "", /缺少 rule-design-ledger-v3/u);
});
