...
formalCase("OPEN-REG-20260803-001", "企业名称必填、字符集与长度边界", async ({ page }) => {
...
formalCase("OPEN-REG-20260803-002", "已注册企业名称拦截", async ({ page }, runtime) => {
  ...
  const registeredName = requireRuntimeValue(
    ...
    "OPEN-REG-20260803-002 的可控已注册合成企业名称"
  );
  expect(registeredName).toBe(approved.corpName);
  ...
  await assertNoMutationRequests(page, async () => {
    await replaceAsUser(
      panel.getByLabel("注册企业名称", { exact: true }),
      registeredName
    );
    await requireUniqueVisible(
...
formalCase("OPEN-REG-20260803-003", "企业标识必填、字符集与长度边界", async ({ page }, runtime) => {
  const panel = await openRegistrationPanel(page);
...
formalCase("OPEN-REG-20260803-004", "已注册企业标识拦截", async ({ page }, runtime) => {
  runtime.classifyFailure("TEST_DATA");
  const approved = await approvedEnterpriseMetadata(runtime);
  const registeredIdentifier = requireRuntimeValue(
    "OPEN_PLATFORM_REGISTERED_COMPANY_IDENTIFIER_TEST",
    "OPEN-REG-20260803-004 的可控已注册合成企业标识",
    /^[a-z0-9]{3,6}$/
  );
  expect(registeredIdentifier).toBe(approved.corpIdentifier);
  runtime.classifyFailure("SCRIPT");
  const panel = await openRegistrationPanel(page);
  ...
  await assertNoMutationRequests(page, async () => {
    await replaceAsUser(
      panel.getByLabel("注册企业标识", { exact: true }),
...
formalCase("OPEN-REG-20260803-005", "统一社会信用代码必填与重复拦截", async ({ page }, runtime) => {
  runtime.classifyFailure("TEST_DATA");
  const approved = await approvedEnterpriseMetadata(runtime);
  const registeredCreditCode = requireRuntimeValue(
    "OPEN_PLATFORM_REGISTERED_CREDIT_CODE_TEST",
    "OPEN-REG-20260803-005 的可控已注册合成统一社会信用代码",
    /^[A-Z0-9]{18}$/
  );
  expect(registeredCreditCode).toBe(approved.corpCode);
  runtime.classifyFailure("SCRIPT");
  const panel = await openRegistrationPanel(page);
...
formalCase("OPEN-REG-20260803-006", "联系方式 11 位与手机号验证入口", async ({ page }, runtime) => {
  runtime.classifyFailure("TEST_DATA");
  const phone = requireRuntimeValue(
    "OPEN_PLATFORM_REGISTRATION_PHONE_TEST",
    "OPEN-REG-20260803-006 的脱敏合成手机号秘密引用",
    /^1[0-9]{10}$/
  );
  runtime.classifyFailure("SCRIPT");
  const panel = await openRegistrationPanel(page);
  ...
  await assertNoMutationRequests(page, async () => {
    ...
    await requireUniqueVisible(
      panel.getByRole("button", { name: "获取注册短信验证码", exact: true }),
      "注册获取验证码按钮"
    );
...
formalCase("OPEN-REG-20260803-011", "企业地址必填与 50 字符边界", async ({ page }, runtime) => {
  runtime.classifyFailure("SCRIPT");
  const panel = await openRegistrationPanel(page);
...
formalCase("OPEN-REG-20260803-012", "申请人姓名必填、字符集与 20 字符边界", async ({ page }, runtime) => {
  runtime.classifyFailure("SCRIPT");
  const panel = await openRegistrationPanel(page);
...
formalCase("OPEN-REG-20260803-013", "企业简介选填与 300 字符边界", async ({ page }) => {
  const panel = await openRegistrationPanel(page);
...
formalCase("OPEN-REG-20260803-014", "企业邮箱选填与格式校验", async ({ page }) => {
  const panel = await openRegistrationPanel(page);
