...
路径 `testpacks/<type>/<project>/<feature>/cases.md`，格式**严格**沿用「输出格式（cases.md 模板）」。硬性要求：

- 顶部引用块四段：默认信息（类型/环境/数据策略）、**上下文来源**（实际读过的路径）、**上下文结论**（影响用例的约束与待确认项）、**范围外声明**（逐项理由，无则写"无"；对象矩阵中因缺凭据/资料/环境无法实现的对象行以"待确认：<缺什么>"列于此）；
- 快速索引表五列（模块/用例编号/用例标题/优先级/风险）；每个用例一个 `<details>` 折叠，标题格式 `编号｜标题｜优先级｜风险`；
...
<details>
<summary><编号>｜<标题>｜<P0/P1/P2>｜<高/中/低>风险</summary>

> 前置条件：<环境、账号、上一用例依赖>。
>
> 是否写入数据：否。 / 是（<写入内容>；生成数据全量明文记录到本功能包 `runtime/` 台账）。**若用例依赖可被其他写用例消耗/推走的前置数据（可删候选、开发态产品、已选模板等），必须同时申报条件性前置写：「当 <耗尽条件> 时，借 <哪个已审核契约> 执行 <补造动作>，产物 <即建即删/保留>、台账 <记哪本>」——该申报经用户 Excel 确认后即为写授权，执行期不再中断请求用户**（先例：product-management 删除用例候选自造、product-function 模板兜底选择；实操教训见 `experience/general.md`「前置数据 + 条件性前置写」节，含台账读取端按删除记录过滤的配套纪律）。
>
> 脚本状态：待审核。
