...
async function appendVerifiedExperience(project: string, scope: string, observation: string, judgment: string, strategy: string, evidence: string, evidenceType: EvidenceType): Promise<string> {
  [scope, observation, judgment, strategy, evidence].forEach((value) => {
    if (!value.trim() || sensitive.test(value)) throw new Error("项目经验字段为空或包含敏感信息。");
  });
  const path = resolve(process.cwd(), "docs/testing/knowledge", `${project}-testing-knowledge.md`);
  await mkdir(resolve(path, ".."), { recursive: true });
  let content: string;
  try { content = await readFile(path, "utf8"); } catch { content = `# ${project} 项目测试经验\n\n`; }
...
async function main(): Promise<void> {
  ...
  if (command === "experience-add" || command === "candidate-promote") {
    ...
    if (!(["受控探索", "正式执行"] as string[]).includes(evidenceType)) throw new Error("--evidence-type 只能为“受控探索”或“正式执行”。");
    const reference = await appendVerifiedExperience(project, required(args, "--scope"), required(args, "--observation"), required(args, "--judgment"), required(args, "--strategy"), required(args, "--evidence"), evidenceType);
    if (command === "candidate-promote") await promoteCandidate(process.cwd(), project, required(args, "--id"), reference, evidenceType);
  ...
  if (command === "candidate-reconcile") {
    ...
    process.stdout.write(JSON.stringify(queue.candidates.map((item) => ({ id: item.id, status: item.status, promotedKnowledgeRef: item.promotedKnowledgeRef })), null, 2) + "\n");
    return;
  }
