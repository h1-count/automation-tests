...
export type KnowledgeDecisionScope = "需求事实" | "通用规则" | "项目经验候选" | "未验证推断";
...
/**
 * Validates the plan-only routing of review findings. It intentionally does
 * not inspect sources, project knowledge files, or reviewer reasoning.
 */
export function validateKnowledgeDecisionRows(
  ...
  if (noFindingRows.length > 0) {
    issues.push("存在发现项或项目经验候选时不得同时填写“无”");
  }
  ...
  if (missingIds.length > 0) {
    issues.push(`缺少发现项沉淀判定：${missingIds.join("、")}`);
  }
  if (duplicateIds.length > 0) {
    issues.push(`发现项重复沉淀判定：${duplicateIds.join("、")}`);
  }
  if (unknownIds.length > 0) {
    issues.push(`沉淀判定引用了不存在的发现项：${[...new Set(unknownIds)].join("、")}`);
  }

  for (const decision of [...findingDecisions, ...candidates]) {
    const finding = findingById.get(decision.findingId);
    if (!finding && !/^EXP-CAND-[0-9]+$/.test(decision.findingId)) {
      continue;
    }
    ...
    if (!finding) {
      if (decision.scope !== "项目经验候选" || !decision.target.includes("plan.md") || decision.target.includes("knowledge/") || decision.evidenceStatus !== "待受控探索或正式执行验证" || decision.disposition !== "待验证") {
        issues.push(`项目经验候选必须留在 plan.md 并等待验证：${decision.findingId}`);
      }
      continue;
    }
