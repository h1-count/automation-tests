...
const dispatchAudit = auditAgentDispatch(plan);
...
const summary = { schema: "test-request-design-summary", reportId: plan.reportId, generatedAt: new Date().toISOString(), planProblems, dispatchProblems: dispatchAudit.problems, dependencyProblems, packs: packResults };
const outputDirectory = path.dirname(planPath);
await writeJsonAtomic(path.join(outputDirectory, "design-summary.json"), summary);
process.stdout.write(`${JSON.stringify({ summary: path.relative(rootDirectory, path.join(outputDirectory, "design-summary.json")), valid: dispatchAudit.valid && planProblems.length === 0 && dependencyProblems.length === 0 && packResults.every((item) => item.problems.length === 0) })}\n`);
if (!dispatchAudit.valid || planProblems.length || dependencyProblems.length || packResults.some((item) => item.problems.length)) process.exitCode = 1;
