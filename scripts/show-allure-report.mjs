...
import { startReportServer, stopReportServer } from "./support/report-server.mjs";
...
await stopReportServer(directories.currentDirectory);
const server = await startReportServer(directories.currentDirectory);
process.stdout.write(`Allure 报告服务已启动：${server.detailUrl}\n`);
