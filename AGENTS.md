...
- 新建或实质变更请求使用 v7：内部完成设计索引、完整用例、校验和评审后只请求一次用例确认；资料冲突、未定义验收和用户修订都进入同一确认。`full_replan` 确认全部用例，`affected_rebuild` 只确认受影响用例，`direct_execute` 不重复确认设计。已有 v5/v6 按原定义恢复，不迁移或改写历史。
- 用例确认不授权业务写入、设备动作、生产访问或安全挑战处理；这些权限仍由独立执行清单确认和环境规范控制。
