...
`testcases/` 存放经过审核的测试设计：测试计划、结构化用例、合理推断和待补充信息。这里回答“测什么、为何这样测”，不存放可执行代码和运行报告。单次请求目录保存自己的 `plan.md` 与 history；经正式收口晋升的长期设计使用 `suites/<feature>/` 无日期目录。
...
├── suite.manifest.json       # stable-test-suite-manifest-v1
├── plan.md                  # 当前稳定设计
└── cases-<module>.md        # 当前稳定用例包
```
...
- 新的功能复测每轮都使用新 `runRequestId`；无变化时通过 suite manifest 引用长期设计，不在 run 目录复制计划和用例。同一 run 中断才恢复原 history，不新建 run。
- `suites/` 不是活跃请求，不包含 `workflow-history.ndjson`、授权、台账或报告，也不被 `reset:full-test-state` 归档。
